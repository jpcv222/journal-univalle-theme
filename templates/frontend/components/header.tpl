...
<body class="pkp_page_{$requestedPage|escape|default:"index"} pkp_op_{$requestedOp|escape|default:"index"}{if $showingLogo} has_site_logo_anulado{/if}" dir="{$currentLocaleLangDir|escape|default:"ltr"}">
...
<div class="pkp_structure_page">
    ...
    {* Header *}
    <header class="pkp_structure_head_anulado" id="headerNavigationContainer" role="banner">
        <div class="pkp_head_wrapper">

            <div class="pkp_site_name_wrapper" style="border-top-style: solid;border-top-width: 6px; border-color:#EB1E00;" >
                <!-- aqui fue -->
                <div id="contenido_nombre_sitio" >
                    ...
                    {/if}
                </div><!-- div nombre sitio -->
            </div><!-- div contenido_nombre_sitio -->
        </div>
        <!--aqui termina -->
        ...
        <nav class="pkp_navigation_user_wrapper navDropdownMenu" id="navigationUserWrapper" aria-label="{translate|escape key="common.navigation.user"}">
            <ul id="navigationUser" class="pkp_navigation_user pkp_nav_list">
                ...
                {else}
                    {if !$disableUserReg}
                        <li id="nav_registro_amx" class="gris_blanco_amx"><a href="{url router=$smarty.const.ROUTE_PAGE page="user" op="register"}">{translate key="navigation.register"}</a></li>
                    {/if}
                    <li id="nav_login_amx" class="gris_blanco_amx"><a href="{url router=$smarty.const.ROUTE_PAGE page="login"}">{translate key="navigation.login"}</a> &nbsp;&nbsp; </li>
                {/if}
            </ul>
        </nav><!-- .pkp_navigation_user_wrapper -->

</div><!-- .pkp_head_wrapper -->
<div id="franja_roja"></div>
...
{/if}
<div class="pkp_structure_content_anulado {if $hasSidebar} has_sidebar{/if}">
    <div id="pkp_content_main" class="pkp_structure_main_anulado" role="main">
